/*
 * ioquake3-PS3: renderer/qgl_ps3.c
 * Defines all qgl* function pointer globals and wires them to ps3gl.
 *
 * Modern ioq3's qgl.h uses GLE macros to declare function pointer typedefs.
 * The actual global variables (qglBindTexture, qglEnable, etc.) are normally
 * defined in sdl_glimp.c via:
 *   #define GLE(ret, name, ...) name##proc * qgl##name = NULL;
 *
 * Since we don't compile sdl_glimp.c, we define them here and assign
 * them to our ps3gl_* implementations in QGL_Init().
 */
...
/* ----------------------------------------------------------------
 * Define all qgl* global function pointer variables.
 * This replicates what sdl_glimp.c does:
 *   #define GLE(ret, name, ...) name##proc * qgl##name = NULL;
 * ---------------------------------------------------------------- */
#define GLE(ret, name, ...) name##proc * qgl##name = NULL;
QGL_1_1_PROCS;
...
/* ----------------------------------------------------------------
 * Extension function pointer globals.
 * These are declared 'extern' in qgl.h but NOT generated by GLE macros.
 * In upstream ioq3, sdl_glimp.c defines them. We must define them here.
 * ---------------------------------------------------------------- */
...
/* ----------------------------------------------------------------
 * QGL_Init -- wire GL 1.1 function pointers to ps3gl layer.
 * GL 1.5+, 2.0+, 3.0+, extension pointers stay NULL (not used
 * by renderergl1).
 * ---------------------------------------------------------------- */
void QGL_Init(void)
{
