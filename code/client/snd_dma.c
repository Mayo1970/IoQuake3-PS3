...
#ifdef __PS3__
static void PS3_RumbleForSfx(const char *name, int entityNum, qboolean localSound)
{
    if (!name || !name[0]) return;

    if (localSound && strstr(name, "feedback/hit")) {
        PS3_SetRumble(80, 140, 90);
        return;
    }
    ...
    if (!Q_stricmpn(name, "sound/weapons/", 14)) {
        ...
        if (strstr(name, "/rocket/")) {
            PS3_SetRumble(255, 220, 160);
            return;
        }
        if (strstr(name, "/shotgun/")) {
            PS3_SetRumble(230, 200, 130);
            return;
        }
        PS3_SetRumble(100, 160, 70);
        return;
    }
}
#endif
...
#ifdef __PS3__
	PS3_RumbleForSfx(sfx->soundName, entityNum, localSound);
#endif
