...
void MSG_WriteDeltaUsercmd( msg_t *msg, usercmd_t *from, usercmd_t *to ) {
	...
	MSG_WriteDelta( msg, from->buttons, to->buttons, 8 );   // 8-bit buttons
...
void MSG_WriteDeltaEntity( msg_t *msg, struct entityState_s *from, struct entityState_s *to,
	...
	if(msg->compat)
	{
		// Compat (proto 43 / EF proto 24): bit vector already encodes which
		// fields changed. No per-field changed/unchanged bit. Write raw
		// values for changed fields.
		for ( i = 0 ; i < numFields ; i++ ) {
			fromF = (int *)( (byte *)from + fields[i].offset );
			toF   = (int *)( (byte *)to   + fields[i].offset );
			if ( *fromF == *toF )
