...
#ifndef MIN_DEDICATED_COMHUNKMEGS
...
void Com_ReadCDKey( const char *filename ) {
	...
	if ( !f ) {
#if defined(__PS3__) && !defined(DEDICATED) && !defined(STANDALONE)
		/* Keep the static cl_cdkey seed so trap_VerifyCDKey passes and
		 * the q3_ui CDKey entry screen is skipped on first boot. */
#else
