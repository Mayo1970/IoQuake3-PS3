...
void R_LoadLightGrid( lump_t *l ) {
	...
	if (r_hdr->integer)
	{
		...
		else if (0)
		{
			...
			for (i = 0; i < numGridPoints; i++)
			{
				w->lightGrid16[i * 6]     = w->lightGridData[i * 8];
				w->lightGrid16[i * 6 + 1] = w->lightGridData[i * 8 + 1];
				w->lightGrid16[i * 6 + 2] = w->lightGridData[i * 8 + 2];
				w->lightGrid16[i * 6 + 3] = w->lightGridData[i * 8 + 3];
				w->lightGrid16[i * 6 + 4] = w->lightGridData[i * 8 + 4];
				w->lightGrid16[i * 6 + 5] = w->lightGridData[i * 8 + 5];
			}
		}
