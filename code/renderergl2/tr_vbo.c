...
void R_VaoPackColor(uint8_t *out, vec4_t c)
{
	out[0] = c[0] * 255.0f + 0.5f;
	out[1] = c[1] * 255.0f + 0.5f;
	out[2] = c[2] * 255.0f + 0.5f;
	out[3] = c[3] * 255.0f + 0.5f;
}
...
vao_t *R_CreateVao2(const char *name, int numVertexes, srfVert_t *verts, int numIndexes, glIndex_t *indexes)
{
	...
	vao->attribs[ATTR_INDEX_COLOR         ].type = GL_UNSIGNED_BYTE;
...
void R_InitVaos(void)
{
	...
	tess.vao->attribs[ATTR_INDEX_COLOR         ].type = GL_UNSIGNED_BYTE;
...
void VaoCache_Init(void)
{
	...
	vc.vao->attribs[ATTR_INDEX_COLOR].type                = GL_UNSIGNED_BYTE;
