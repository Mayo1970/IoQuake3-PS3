...
static void RB_SurfacePolychain( srfPoly_t *p ) {
	...
	for ( i = 0; i < p->numVerts; i++ ) {
		...
		tess.color[numv][0] = (int)p->verts[i].modulate[0];
		tess.color[numv][1] = (int)p->verts[i].modulate[1];
		tess.color[numv][2] = (int)p->verts[i].modulate[2];
		tess.color[numv][3] = (int)p->verts[i].modulate[3];
...
static void DoRailCore( const vec3_t start, const vec3_t end, const vec3_t up, float len, float spanWidth )
{
	...
	tess.color[tess.numVertexes][0] = backEnd.currentEntity->e.shaderRGBA[0] * 0.25f;
	tess.color[tess.numVertexes][1] = backEnd.currentEntity->e.shaderRGBA[1] * 0.25f;
	tess.color[tess.numVertexes][2] = backEnd.currentEntity->e.shaderRGBA[2] * 0.25f;
	tess.numVertexes++;
